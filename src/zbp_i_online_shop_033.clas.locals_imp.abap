CLASS lhc_ZI_ONLINE_SHOP_033 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_ZI_ONLINE_SHOP_033 IMPLEMENTATION.
     ...
     CREATE BY \_purchaserequisitionitem
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API 033 - Workshop prep.'
                                        ) (
                                          %cid           = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API 033'
                                        ) )
              )   )
      REPORTED DATA(ls_pr_reported)
      MAPPED DATA(ls_pr_mapped)
      FAILED DATA(ls_pr_failed).
    zbp_i_online_shop_033=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_033,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_033.

    READ ENTITIES OF zi_online_shop_033 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
   MODIFY ENTITIES OF zi_online_shop_033 IN LOCAL MODE
    ENTITY zi_online_shop_033 UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_033~create_pr,
...
CLASS lsc_ZI_ONLINE_SHOP_033 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_ZI_ONLINE_SHOP_033 IMPLEMENTATION.

  METHOD save_modified.

    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_033,

           ls_online_shop_as TYPE                   zshop_as_033.

    IF zbp_i_online_shop_033=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.

      LOOP AT zbp_i_online_shop_033=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_033 FROM TABLE @lt_online_shop_as.
